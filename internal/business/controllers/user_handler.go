...
func UpdateWishlist(c *gin.Context) {
	...
	_, err = db.Replace("users", bson.M{"_id": objectId}, usuario)
	if err != nil {
		c.IndentedJSON(http.StatusInternalServerError, data.JsonError{Message: err.Error()})
		return
	}
	...
	updatedUser, err := db.Get[data.User]("users", bson.M{"_id": objectId})
	if err != nil {
		c.IndentedJSON(http.StatusInternalServerError, data.JsonError{Message: err.Error()})
		return
	}
	c.IndentedJSON(http.StatusOK, updatedUser.Wishlist)
}
